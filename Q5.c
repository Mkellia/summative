...
void compressFile() {
    ...
    if (!in) { printf("❌ Error reading input file.\n"); exit(1); }
    ...
    fclose(in);
    fclose(out);
    printf(" Compression complete: patient_record.txt → compressed.huff\n");
}

void decompressFile() {
    FILE* in = fopen("compressed.huff", "rb");
    FILE* tree = fopen("tree.bin", "rb");
    FILE* out = fopen("decompressed.txt", "w");

    if (!in || !tree || !out) { printf(" Error opening files.\n"); exit(1); }
    ...
    fclose(tree);
    fclose(out);
    printf(" Decompression complete: compressed.huff → decompressed.txt\n");
}
...
int main() {
    ...
    printf("2. Decompress 'compressed.huff'\n");
